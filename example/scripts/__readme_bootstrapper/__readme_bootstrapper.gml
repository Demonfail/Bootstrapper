...
//     
//     1) The package must use gml_pragma( "global", ... ) to execute a primary script;
//     2) The primary script must only define variables essential to the package's operation;
//     3) If further processing is needed, the primary script may queue a script into the bootstrapper;
...
//  It allows for code to be executed before the game fully boots - we call this the "primary script".
//  The global pragma command means code is run before the first room. This is handy because it means a
...
//  developers use the their primary script (see Step 1) for the bare minimum initialisation of a package.
